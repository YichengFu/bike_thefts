...
date-format: long
toc: true
abstract: "This analysis investigates bicycle theft in Toronto, emphasizing high-risk neighborhoods. Using Bayesian logistic regression, the study identifies key predictors, including premises type, bike cost, and time of day. Results reveal that theft likelihood increases in public spaces and during evening hours, with high-cost bicycles as prime targets. Specific neighborhoods consistently report elevated theft rates, suggesting localized vulnerabilities. These findings support targeted interventions, such as improved security measures and community awareness, to mitigate theft risks and enhance urban safety."
format: pdf
...
Bicycle theft is a persistent issue in urban environments, imposing economic and emotional burdens on individuals and communities. As cities like Toronto increasingly embrace cycling as a sustainable transportation mode, mitigating theft is essential for maintaining public trust in cycling infrastructure. However, existing studies often focus on general crime trends, overlooking the nuanced factors that drive bicycle theft in specific high-risk neighborhoods.

This study addresses this gap by analyzing reported bicycle thefts in Toronto. Using statistical modeling, it identifies contextual factors such as premises type, time of occurrence, and bike cost that influence theft likelihood in high-risk neighborhoods. By focusing on these areas, the research highlights actionable risk factors and proposes strategies to prevent theft and enhance cyclist security.

## Paper Structure:

The remainder of this paper is structured as follows. In @sec-data, the overview of the data used in this study and the variables of interests will be introduced. Further the data normalization will be discussed in details. @sec-model illustrates the Bayesian logistic regression model built in our analysis, some details include model set up, assumptions and justification. @sec-result highlights the result of the model visualizing using tables and graphs. Lastly, @sec-discussion contains discussion of the analysis based on findings, the limitations of the model and the suggestion for future research. @sec-survey contains information about the survey as well as its design and limitations. @sec-data-details contains more information about the original data and its visualizations. @sec-model-details shows more plots and information about the model built for this research. 
...
The results of the Bayesian logistic regression model emphasize the role of contextual and temporal factors in shaping bicycle theft patterns. Key findings include:

- Premises Type: Theft risk is significantly higher in public spaces such as streets and parks compared to residential premises, likely due to lower security measures.
- Time of Day: The likelihood of theft increases during evening hours, reflecting reduced visibility and activity.
- Bike Cost: High-cost bicycles are more frequently targeted, underscoring their appeal to thieves.
...
These findings suggest actionable strategies for reducing bicycle theft in urban areas by addressing vulnerabilities in public spaces. High-risk locations, such as streets, parks, and transit areas, often lack adequate security measures, making them prime targets for theft. Implementing targeted interventions, such as enhanced lighting, can deter criminal activity by increasing visibility and discouraging opportunistic behavior. Community policing, involving regular patrols and partnerships with local cycling groups, can create a sense of security and deter theft through increased accountability. Secure parking zones equipped with features such as CCTV surveillance, well-lit areas, and designated bike racks can provide cyclists
...
# Idealized Survey Methodology {#sec-survey}
